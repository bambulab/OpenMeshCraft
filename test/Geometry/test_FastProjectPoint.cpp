...
class test_FastProjectPoint : public testing::Test
{
protected:
	...
	bool pnts_equal(const Our_Pnt3 &t, const CGAL_Pnt3 &s)
	{
		for (size_t i = 0; i < 3; i++)
		{
			...
			{
				std::cout << std::format("{}-th elements are not equal: {} != {}.", i,
				                         t[i], s[(int)i]);
				return false;
			}
		}
...
TEST_F(test_FastProjectPoint, Projection)
{
	...
	for (size_t i = 0; i < 10000; i++)
	{
		...
		CGAL_Pnt3 project_p = project.operator()(cgal_tri, query_p);
		Our_Pnt3  project_v = our_fastprojct(our_tri, query_v);
