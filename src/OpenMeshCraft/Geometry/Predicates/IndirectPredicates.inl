                                               ...
                                               const PointT &c, const PointT &d)
{
  if (a.is_explicit() && b.is_explicit() && c.is_explicit() && d.is_explicit())
    return inTriangleCircumSphere<TP>(a, b, c, d);

  OMC_ASSERT(false, "inTriangleCircumSphere - should not happen");
  return Sign::ZERO; // warning killer
}
...
Sign InSphere_Indirect<FT, IT, ET>::operator()(const FT *a, const FT *b,
                                               const FT *c, const FT *d)
{
  return inTriangleCircumSphere<TP>(a[0], a[1], a[2], b[0], b[1], b[2], c[0],
                                    c[1], c[2], d[0], d[1], d[2]);
}
...
Sign InSphere_Indirect<FT, IT, ET>::operator()(const FT *a, const FT *b,
                                               const FT *c)
{
  return inSegmentDiametricalSphere<TP>(a[0], a[1], a[2], b[0], b[1], b[2],
                                        c[0], c[1], c[2]);
}
