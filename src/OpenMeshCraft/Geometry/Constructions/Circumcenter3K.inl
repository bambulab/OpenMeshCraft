...
namespace OMC {
template <typename K, bool F, bool FE>
auto ConstructCircumcenter3K<K, F, FE>::operator()(const EPoint3 &v0,
                                                   const EPoint3 &v1) const
  -> EPoint3
{
  return (v0 + v1) * 0.5;
}
...
template <typename K, bool F, bool FE>
auto ConstructCircumcenter3K<K, F, FE>::operator()(const Segment3 &seg) const
  -> EPoint3
{
  ...
  -> EPoint3
{
  ...
  if constexpr (!Filtered && !ForceExact)
  ...
  if constexpr (!ForceExact)
  {
    // Filter the result
    if (!(den == NT(0)))
    {
      NT      inv = NT(1) / (NT(2) * den);
      EPoint3 res(v0.x() + numx * inv, v0.y() - numy * inv,
                  v0.z() + numz * inv);
...
template <typename K, bool F, bool FE>
auto ConstructCircumcenter3K<K, F, FE>::operator()(const Triangle3 &tri) const
  -> EPoint3
{
...
template <typename K, bool F, bool FE>
auto ConstructCircumcenter3K<K, F, FE>::operator()(
...
auto ConstructCircumcenter3K<K, F, FE>::operator()(const EPoint3 &v0,
                                                   const EPoint3 &v1,
                                                   const EPoint3 &v2,
                                                   const EPoint3 &v3, NT w0,
                                                   NT w1, NT w2, NT w3) const
  -> EPoint3
{
