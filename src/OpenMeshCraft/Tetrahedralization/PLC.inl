...
PiecewiseLinearComplex<Traits>::PLCEdge::PLCEdge(
  PLCEdgeType _type, index_t e0, index_t e1, index_t _ancestor_id,
...
/**
 * @brief Remove isolated steiner vertex `iso_vid` (generated by collapsing
 * edge) by replacing it with the last vertex and removing the last one.
 * @param iso_vid The isolated Steiner vertex
 */
template <typename Traits>
...
/**
 * @brief Retrieves the parent edge ID of a given edge.
 * If the parent edge ID is not valid, it returns the ID of the edge itself.
 */
template <typename Traits>
...
/**
 * @brief Collapses two sub-edges of a Piecewise Linear Complex (PLC) into their
 * parent edge. The collapsed edges are removed by swaping to the end of PLC
 * edges and poping. The split vertex is isolated and replaced by the last
 * vertex. All related data in the PLC are updated after collapsing.
 * @param ep0 The first edge index to collapse.
 * @param ep1 The second edge index to collapse.
 * @return The index of the split vertex of the collapsed edges, which will be
 * isolated after collapsing. It should be removed (or recycled) after
 * collapsing.
 */
template <typename Traits>
...
/**
 * @brief Removes isolated edges from the PLC by overwriting it with the last
 * edges and removing the last edges.
 * @param e0 The first edge index to remove.
 * @param e1 The second edge index to remove.
 */
template <typename Traits>
